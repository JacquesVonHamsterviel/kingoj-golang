...
new setAttrib({id:'tb1',cor1:'#DDDDE4',cor2:'#BAD0FC'});
...
for(i=1;i<RS.length;i++){
	...
	var probName=TD[2].innerText.trim();
	TD[2].innerHTML='<a href=\x22prob/' + probName + '\x22 target=_blank>☆</a> <a href=\x22prob/' + probName + '/prob.html\x22 target=_blank>' + probName + '</a>';
	var result=TD[3].innerText.trim();
	TD[3].innerHTML='<a href=\x22submit/' + probid+ '/prog.pas\x22 target=_blank>' + result + '</a>';
}
...
function hiddNotRight(){
	...
	var RS=$('tb1').rows;
	for(i=1;i<RS.length;i++){
		var result=RS[i].getElementsByTagName('td')[3].innerText.trim();
		if(result!='Accepted')
